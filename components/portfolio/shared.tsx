import type { SVGProps } from 'react'
import { cn } from '@/lib/utils'

export const LINKS = {
  github: 'https://github.com/PrincessC1',
  linkedin: 'https://www.linkedin.com/in/princess-chishato-7b6368183',
  email: 'mailto:rumbidzaikc@gmail.com',
  voicebot: 'https://github.com/PrincessC1/patient-voicebot',
}

export const external = { target: '_blank', rel: 'noopener noreferrer' } as const

export function GitHubIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M12 .5C5.73.5.75 5.48.75 11.75c0 4.97 3.22 9.18 7.69 10.67.56.1.77-.24.77-.54v-1.9c-3.13.68-3.79-1.51-3.79-1.51-.51-1.3-1.25-1.65-1.25-1.65-1.02-.7.08-.69.08-.69 1.13.08 1.72 1.16 1.72 1.16 1 1.72 2.64 1.22 3.28.93.1-.73.39-1.22.71-1.5-2.5-.28-5.13-1.25-5.13-5.57 0-1.23.44-2.24 1.16-3.03-.12-.28-.5-1.43.11-2.98 0 0 .95-.3 3.1 1.16a10.8 10.8 0 0 1 5.64 0c2.15-1.46 3.1-1.16 3.1-1.16.61 1.55.23 2.7.11 2.98.72.79 1.16 1.8 1.16 3.03 0 4.33-2.64 5.28-5.15 5.56.4.35.76 1.03.76 2.08v3.09c0 .3.2.65.78.54a11.26 11.26 0 0 0 7.68-10.67C23.25 5.48 18.27.5 12 .5Z" />
    </svg>
  )
}

export function LinkedInIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.48-.9 1.64-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.46v6.28ZM5.34 7.43a2.06 2.06 0 1 1 0-4.13 2.06 2.06 0 0 1 0 4.13ZM7.12 20.45H3.56V9h3.56v11.45ZM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.73V1.73C24 .77 23.2 0 22.22 0Z" />
    </svg>
  )
}

export function SectionHeading({
  index,
  label,
  title,
  id,
}: {
  index: string
  label: string
  title: string
  id: string
}) {
  return (
    <div className="mb-12 md:mb-16">
      <div className="flex items-center gap-4 font-mono text-xs tracking-[0.2em] text-primary uppercase">
        <span>{index}</span>
        <span className="h-px w-12 bg-primary/60" aria-hidden="true" />
        <span className="text-muted-foreground">{label}</span>
      </div>
      <h2 id={id} className="mt-4 text-3xl font-semibold tracking-tight text-balance md:text-4xl">
        {title}
      </h2>
    </div>
  )
}

export function Tag({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <li
      className={cn(
        'rounded-md border border-border bg-secondary/60 px-2.5 py-1 font-mono text-xs text-muted-foreground',
        className,
      )}
    >
      {children}
    </li>
  )
}

export function TagList({ tags, label }: { tags: string[]; label: string }) {
  return (
    <ul className="flex flex-wrap gap-2" aria-label={label}>
      {tags.map((tag) => (
        <Tag key={tag}>{tag}</Tag>
      ))}
    </ul>
  )
}

export function BulletList({ items }: { items: React.ReactNode[] }) {
  return (
    <ul className="space-y-3">
      {items.map((item, i) => (
        <li key={i} className="flex gap-3 text-pretty leading-relaxed text-muted-foreground">
          <span className="mt-2.5 size-1.5 shrink-0 rounded-full bg-primary/70" aria-hidden="true" />
          <span>{item}</span>
        </li>
      ))}
    </ul>
  )
}
